...
switch (_typeX) do
	...
	//case "I_NORTH_NOR_Officer": {_textX = "Commander role.<br/><br/>Commanders have no bonuses or disadvantages."};
	case "I_NORTH_NOR_Medic":  {player setUnitTrait ["audibleCoef",1.1]; player setUnitTrait ["loadCoef",0.7]; _textX = "Medic role.<br/><br/>Medics, while being louder when on the move, are able to carry more than your usual rifleman."}; //reintroduced - 8th January 2020, Bob Murphy
	case "I_NORTH_NOR_Rifleman_SGT": {player setUnitTrait ["camouflageCoef",0.8]; player setUnitTrait ["loadCoef",1.4]; _textX = "Teamleader role.<br/><br/>Teamleaders can carry less in exchange for more camouflage."}; //reintroduced - 8th January 2020, Bob Murphy
	case "I_NORTH_NOR_Rifleman":  {player setUnitTrait ["audibleCoef",0.8]; player setUnitTrait ["loadCoef",1.1]; player setUnitTrait ["UAVHacker",true]; _textX = "Rifleman role.<br/><br/>Riflemen are more suitable to silent sneak, but have less carrying capacity"}; //reintroduced - 8th January 2020, Bob Murphy
	case "I_NORTH_NOR_Scout": {player setUnitTrait ["camouflageCoef",0.7]; player setUnitTrait ["loadCoef",1.3]; _textX = "Marksman role.<br/><br/>Marksmen are able to carry less in exchange for being harder to spot."}; //reintroduced - 8th January 2020, Bob Murphy
	case "I_NORTH_NOR_Machinegunner": {player setUnitTrait ["audibleCoef",1.2]; player setUnitTrait ["loadCoef",0.8]; player setUnitTrait ["camouflageCoef",1.1]; _textX = "Machinegunner role.<br/><br/>Machinegunners have a slight bonus on carry capacity, but make more noise when they move and are ever-so slightly more easy to spot."}; //reintroduced - 8th January 2020, Bob Murphy
	case "I_NORTH_NOR_Engineer":  {_textX = "Engineer role.<br/><br/>Engineers do not have any bonus or penalties, but have the ability to use Repair Kits for vehicle repair"}; //reintroduced - 8th January 2020, Bob Murphy
	case "I_NORTH_NOR_Lieutenant":  {_textX = "Commander role.<br/><br/>Commanders have no bonuses or disadvantages."}; //mmmm
